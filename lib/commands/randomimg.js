...
module.exports.randomimg = async function (message, bot, nsfw = "") {
...
module.exports.randomimg.cat = "anime";
...
module.exports.lewd.cat = "anime";

module.exports.imgtag = async function (message, bot, ...args) {
    let newMsg = await message.channel.send('Buscando...');
...
module.exports.imgtag.cat = "anime";

module.exports.img = async function (message, bot, ...args) {
    let newMsg = await message.channel.send('Buscando...');
    tagrandom(newMsg, args)
};

module.exports.img.description = "Busca una imagen según la categoria";
module.exports.img.args = "<tag>";
module.exports.img.cat = "anime";
...
async function tagrandom(message, tag, pagemax = 1000) {
    ...
    let pageNumber = parseInt(Math.random() * (pagemax - 1) + 1);
    ...
    axios.get(pageURL).then(async (result) => {
        ...
        if (images.length == 0) {
            ...
            if (contentWiki.length < 3 || pageNumber == 0) {
                ...
                tagrandom(message, tag, pageNumber - 1);
                return;
            }
        }
